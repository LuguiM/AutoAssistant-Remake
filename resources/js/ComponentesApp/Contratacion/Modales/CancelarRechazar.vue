    ...
    <v-dialog v-model="dialog" persistent width="auto">
        <template v-slot:activator="{ props }" v-if="prop.type === 'text'">
            ...
            <v-btn v-else v-bind="props" class="bg-error" icon>
                ...
                <v-tooltip activator="parent" location="top">Cancelar servicio</v-tooltip>
            </v-btn>

        </template>

        <template v-slot:activator="{ props }" v-else="prop.type ==='icon'">
            <v-btn v-if="prop.title === 'Rechazar'" v-bind="props" variant="flat" density="compact"
        ...
        <v-card class="text-center bg-greyDark">
            ...
            <v-card-text class="text-h4" v-if="prop.title === 'Cancelar'"><v-icon
                    color="error">mdi-delete-outline</v-icon></v-card-text>
            <v-card-text class="text-h4" v-else><v-icon color="error">mdi-close-circle-outline</v-icon></v-card-text>

            <v-card-text>¿Esta seguro de querer <span class="text-lowercase">{{ prop.title }}</span> este
                servicio?</v-card-text>
            <v-card-text v-if="prop.title === 'Rechazar'" class="pt-0">Deje un motivo para poder rechazar el servicio
            </v-card-text>
            <v-card-text v-else class="pt-0">Esta accion sera irreversible</v-card-text>

            <v-card-text v-if="prop.title === 'Rechazar'">
                <v-textarea v-model="form.motivo" label="Motivo" variant="solo" auto-grow rows="4" row-height="30">
                </v-textarea>
            </v-card-text>
